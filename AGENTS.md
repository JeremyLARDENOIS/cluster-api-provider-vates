...
examples/*               Example CRs (edit these)
Makefile                       Build/test/deploy commands
Makefile.dev                   Build and deploy on kind cluster (local)
...
kubebuilder create api --group example.com --version v1beta2 --kind Memcached \
...
```bash
make -f Makefile.dev build    # Build image via $(CONTAINER_TOOL)
make -f Makefile.dev push     # Load image into kind cluster
make -f Makefile.dev deploy   # kubectl apply via kustomize
make -f Makefile.dev restart  # Restart controller deployment
```
...
**Two-field pattern (ID + Name):**

Use two separate fields for resources that accept either a UUID or a name:
...
type Network struct {
	NetworkID string `json:"networkID,omitempty"` // UUID
	Name      string `json:"name,omitempty"`      // name (alternative)
}
```

Resolve functions check the ID field first (no API call), then fall back to name lookup:

```go
if poolIDStr != "" {
    return uuid.FromString(poolIDStr) // direct, no API fetch
}
if poolName != "" {
    // v1Client.GetPoolByName(poolName) → API fetch
}
```
...
```bash
kubebuilder create api --group example --version v1beta2 --kind MyApp \
