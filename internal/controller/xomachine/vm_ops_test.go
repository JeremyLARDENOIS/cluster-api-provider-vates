...
var _ = Describe("ManageVIFs", func() {
	...
	It("does nothing when NetworkConfig is nil", func() {
		err := ManageVIFs(context.Background(), nil, &infrastructurev1beta2.XOMachine{}, &payloads.VM{})
		Expect(err).NotTo(HaveOccurred())
	})

	It("does nothing when Networks is empty", func() {
		err := ManageVIFs(context.Background(), nil, &infrastructurev1beta2.XOMachine{
